...
- a high majority of all imported rows successfully normalize into a selectable exercise (the quality tolerance lives here — in how much of the raw import becomes usable — not in what "usable" requires). Run against the real ~4.2k-row merge across 4 Kaggle sources (2026-08-20): started at 50.4%, reached 62.6% after expanding the `movementPattern` taxonomy from 13 to 18 values (added `knee_extension`, `shoulder_rotation`, `shoulder_elevation`, `ankle_plantarflexion`, `core_rotation` — real, previously-uncovered movement categories, not filler) and broadening keyword-rule coverage. The residual ~37% is not a rule-coverage gap: it's dominated by boxing/conditioning-circuit movements (punches, burpee combos — genuinely outside a strength-training movement-pattern taxonomy) and angle-ambiguous press variants where a blanket keyword rule would misclassify horizontal vs. vertical push. An original ≥95% figure was set before any real data was in hand and turned out to be unrealistic without trading away classification correctness — do not force further coverage by loosening keyword specificity; further gains here should come from manual review of `catalog_review.csv`-style ambiguous cases, not broader substring rules,
